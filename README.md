# Hi, I'm Niketa Dangi 👋

## Aspiring Data Analyst

Passionate about transforming raw data into meaningful insights using Python, Power BI, SQL, and Excel.
Currently building real-world analytics projects focused on data cleaning, visualization, dashboard creation, and business insights.

---

## Skills & Tools

* Python
* SQL
* Power BI
* Microsoft Excel
* Pandas
* NumPy
* Data Cleaning
* Data Visualization
* Exploratory Data Analysis (EDA)
* AI

---

## Featured Project

### Fraud Transaction Analysis

A data analytics project focused on identifying fraudulent financial transactions using Python and Power BI.

#### Project Highlights

* Cleaned and processed transaction datasets
* Performed exploratory data analysis
* Identified fraud patterns and trends
* Built an interactive Power BI dashboard
* Generated actionable insights from transaction data

#### Tools Used

Python | Pandas | NumPy | Power BI | Excel

#### Project Link

Fraud Transaction Analysis Project - https://github.com/Niketa-dangi/fraud-transaction-analysis-project

Python Learning Journey Repository Link

Python Learning Journey - https://github.com/Niketa-dangi/python-learning-journey

---

## Education

**B.Sc (PCM)** — 2022 to 2025
Currently pursuing **M.Sc (Mathematics)**

---

## Goals

* Strengthening Data Analytics skills
* Building real-world portfolio projects
* Learning advanced Power BI and SQL
* Exploring Machine Learning fundamentals

---

## Resume

Download Resume - https://github.com/Niketa-dangi/Niketa-dangi/blob/main/Niketa_Dangi_Resume.pdf.pdf

---

## Connect With Me

### LinkedIn

www.linkedin.com/in/niketa-dangi-bbb3533bb

### GitHub

https://github.com/Niketa-dangi

---

⭐ Always open to learning, collaboration, and new opportunities in Data Analytics.

